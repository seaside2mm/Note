...
var en = {
    ...
    thisPluginName: 'Enhanced Editing ',
    ...
    FunctionUpdate: 'Version 0.5.1 adds a bilingual interface between Chinese and English, abolition of the "Smart line" function, the conversion of English and Chinese punctuation features optimized for the "Repair error Punctuation" ...',
...
var zhCN = {
    ...
    SupportBatchConversion: '支持批量转换多行文本（需用换行符分隔）或多句文本（需用顿号分隔）。',
    ...
    ConvertCursorJump: '📣 键控光标跳转「Alt+Shift +J, +L」',
...
class MyPlugin extends obsidian.Plugin {
    ...
    onload() {
        ...
        */
         ...
         */
        this.addCommand({
            id: 'add-warning-callout',
            name: '> [!warning]',
            callback: () => this.addWarningCallout("warning", "#中参考价值","该文档缺少可靠信息源"),
        ...
        this.addCommand({
            id: 'add-bug-callout',
            name: '> [!bug]',
            callback: () => this.addWarningCallout("bug", "#低参考价值", "此文档没有列出任何参考或来源"),
         ...
         */
        this.addCommand({
            id: 'add-quote-callout',
            name: '> [!quote]',
            callback: () => this.addWarningCallout("quote", "", ""),
    ...
    实用命令菜单() {
        ...
        this.registerDomEvent(this.statusBarIcon, "click", (e) => {
            ...
            menu.addItem((item) => {
                item.setTitle("行首缩进两字符");
                item.setIcon("indent-glyph");
                item.onClick(() =>this.行首缩进两字符());
            });
            menu.addItem((item) => {
            ...
            menu.addItem((item) => {
                item.setTitle("折叠当前同级标题");
                item.setIcon("double-up-arrow-glyph");
                item.onClick(() =>this.折叠当前同级标题());
            });
            menu.addItem((item) => {
    ...
    批量转换内链() {
        ...
        for (const oldStr of fileNameAry){
            ...
            笔记正文 = 笔记正文.replace(oldStr,"[["+xinStr+"]]");
        };
        笔记正文 = 笔记正文.replace(/⚘/g,"");
        this.替换笔记正文(笔记正文);
        new obsidian.Notice("潜在链接已全部转换！\n可以 Ctrl+Z 撤消。");
    };
     ...
     * @param state
     */
    addWarningCallout(state, title, info) {
        this.获取编辑器信息 ();
        笔记全文.replaceRange("> [!" + state + "] " + title + "\n" + "> " + info + "\n> 详情可看 [[信息可信度标准]]\n", 当前光标, 当前光标);
        编辑模式.exec("goDown");
    }
    ...
    标题语法(_str) {
        ...
        if(_str==""){   //若为标题，转为普通文本
            ...
            新定位 = 选至行首.replace(/^(\>*(\[[!\w]+\])?\s*)#+\s/,"$1");
        } else if (link.test(当前行文本)){   //已是同级标题，转为普通文本
            新文本 = 当前行文本.replace(link,"$1");
            新定位 = 选至行首.replace(link,"$1");
        } else{  //不管是标题还是普通文本，都转为标题
            新文本 = 当前行文本.replace(/^(\>*(\[[!\w]+\])?\s*)(#+\s)?/m,"$1"+_str+" ");
            新定位 = 选至行首.replace(/^(\>*(\[[!\w]+\])?\s*)(#+\s)?/m,"$1"+_str+" ");
        }
    ...
    内链转为超链接() {
		this.获取编辑器信息 ();
        if (!笔记正文) return;
        笔记正文 = 笔记正文.replace(/\[\[([^\[\]]+)(\.\w{3,4})?\]\]/g,"[$1]($1$2)");
        笔记正文 = 笔记正文.replace(/(?<=\]\([^\)]*)\s(?=[^\(]*\))/g,"%20");
        this.替换笔记正文 (笔记正文);
    };
    ...
    修复错误标点() {
        this.获取编辑器信息 ();
        if (!笔记正文) return;
        笔记正文 = 笔记正文.replace(/(?<=[一-龥]),(?=[一-龥])/g,"，");
        笔记正文 = 笔记正文.replace(/(?<=[一-龥])\.(?=[一-龥])/g,"。");   //数字后的黑点不转为句号
        笔记正文 = 笔记正文.replace(/(?<=[一-龥])\?(?=[一-龥])/g,"？");
        笔记正文 = 笔记正文.replace(/(?<=[一-龥]):(?=[一-龥])/g,"：");
        笔记正文 = 笔记正文.replace(/(?<=[一-龥]);(?=[一-龥])/g,"；");
        笔记正文 = 笔记正文.replace(/(?<=[一-龥]) (?=[一-龥])/g,"");
        ...
        笔记正文 = 笔记正文.replace(/(?<=[^一-龥])：(?=[^一-龥])/g,":");
        笔记正文 = 笔记正文.replace(/(?<=[^一-龥])；(?=[^一-龥])/g,";");
        笔记正文 = 笔记正文.replace(/(?<=[\(（)])[^一-龥]+(?=[\)）])/g,"($1)");
        笔记正文 = 笔记正文.replace(/(?<=[\(（)]).*?[一-龥].*?(?=[\)）])/g,"（$1）");
        笔记正文 = 笔记正文.replace(/：：/g,"::");
        笔记正文 = 笔记正文.replace(/【【/g,"[[");
        笔记正文 = 笔记正文.replace(/】】/g,"]]");
        笔记正文 = 笔记正文.replace(/(?<=[,\.?!])([^\s])/g," $1");
        this.替换笔记正文 (笔记正文);
    };
...
class editSettingsTab extends obsidian.PluginSettingTab {
    ...
    display() {
        ...
        containerEl.empty();
        containerEl.createEl('h2', { text: t('thisPluginName')+当前版本 });
        new obsidian.Setting(containerEl)
            .setName(t('convertInternalLink'))
            .setDesc(t('SupportBatchConversion'))
        ...
        linkText.appendText(t('ConvertSynonymousLink'))
        ...
        */
        ...
        new obsidian.Setting(containerEl)
            .setName(t('setInterTab'))
            .setDesc(t('whenEnabledTab'))
        ...
        new obsidian.Setting(containerEl)
            .setName(t('setSmartSyntax'))
            .setDesc(t('isYouCanCombine'))

        new obsidian.Setting(containerEl)
            .setName(t('setSmartPaste'))
            .setDesc(t('isPasteTableURL'))

        new obsidian.Setting(containerEl)
            .setName(t('ConvertCursorMove'))
            .setDesc(t('isConvertCursorMove'))

        new obsidian.Setting(containerEl)
            .setName(t('ConvertCursorJump'))
            .setDesc(t('isConvertCursorJump'))
        ...
        mdText.appendChild(document.createElement('br'));
        mdText.appendText('转换上标语法「未设置」∶将选文转为或去除 <sup>上标</sup> 效果');
        mdText.appendChild(document.createElement('br'));
        mdText.appendText('转换下标语法「未设置」∶将选文转为或去除 <sub>下标</sub> 效果');
        mdText.appendChild(document.createElement('br'));
        ...
        charText.appendText('修复错误标点「未设置」∶将笔记中的汉字中间的英文标点修复为中文标点');
        ...
        toolText.appendChild(document.createElement('br'));
        toolText.appendText('选择当前语法「Alt+Shift+K」：选择光标所在的当前MrakDown语法（如加粗、高亮、删除、链接等效果）文本。');
        toolText.appendChild(document.createElement('br'));
        ...
        lineText.appendChild(document.createElement('br'));
        lineText.appendText('行首缩进两字符「未设置」∶在每行文本的行首添加两个全角空格，产生缩进效果。');
        lineText.appendChild(document.createElement('br'));
